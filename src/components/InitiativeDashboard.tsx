      ...
      <div className="px-4 sm:px-6 pt-5 pb-2">
        <div className="max-w-6xl mx-auto initiative-island flex items-center justify-between gap-3 px-4 py-3">
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-[0.22em] text-teal-800/60 font-semibold">
              Caisse
            </p>
            <h1 className="text-lg sm:text-xl font-semibold tracking-tight text-teal-950 truncate">
              Initiative-Judo
            </h1>
          </div>
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            <div className="hidden md:block text-right mr-2">
              <p className="text-sm font-medium text-teal-950">{user.name}</p>
              <p className="text-xs text-teal-800/60">{roleLabel(user.role)}</p>
            </div>
            {onChangeWorkspace && (
              <button
                type="button"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[240px_minmax(0,1fr)] gap-6 lg:gap-8">
          ...
          <aside className="hidden lg:flex flex-col gap-4 initiative-rise">
            <div className="initiative-rail p-5">
              <p className="text-[10px] uppercase tracking-[0.2em] text-teal-800/55 font-semibold mb-4">
                Synthèse
              </p>
              {stats && (
              ...
              {onOpenRecap && (
                <button
                  type="button"
                  onClick={onOpenRecap}
                  className="mt-6 w-full py-3 text-sm font-semibold text-teal-950 border border-teal-900/15 hover:bg-teal-900/5 transition-colors"
          ...
          <div className="space-y-6 min-w-0">
            ...
            <section className="initiative-rise-delay">
              <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-5">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-teal-800/55 font-semibold">
                    Mouvements
                  </p>
                  <h2 className="text-2xl font-semibold tracking-tight text-teal-950 mt-1">
                    Historique
                  </h2>
                </div>
                <div className="flex gap-0 border border-teal-900/15 overflow-hidden">
                    ...
                    <button
                      ...
                      className={`px-3.5 py-2 text-xs font-semibold uppercase tracking-wide transition-colors ${
                        filter === f
                          ...
                          : "bg-white/50 text-teal-900/70 hover:bg-white"
                      }`}
                    >
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              {tx.type === "entree" ? (
                                <ArrowDownLeft className="w-4 h-4 text-teal-700 flex-shrink-0" />
                              ) : (
                            ...
                            <p className="text-xs sm:text-sm text-teal-800/65">
                              {formatDate(tx.date)}
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            {canDelete && (
                              <button
                                ...
                                className="p-2 text-teal-900/40 hover:text-orange-700 hover:bg-orange-50 transition-colors disabled:opacity-50"
                                title="Supprimer"
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between gap-3 mt-5 pt-4 border-t border-teal-900/10">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium border border-teal-900/15 bg-white/60 hover:bg-white disabled:opacity-40"
                    >
...
function RailMetric({
  ...
  return (
    <div>
      <p className="text-[10px] uppercase tracking-[0.16em] text-teal-800/50 font-semibold">
        {label}
      </p>
  ...
  return (
    <div
      className={`flex-shrink-0 min-w-[140px] px-4 py-3 border ${
        tone === "in"
          ? "border-teal-700/25 bg-teal-50/80"
          : tone === "out"
            ? "border-orange-700/25 bg-orange-50/80"
            : "border-teal-900/15 bg-white/70"
      }`}
      style={{ clipPath: "polygon(0 0, 100% 0, 100% 78%, 88% 100%, 0 100%)" }}
    >
      <p className="text-[10px] uppercase tracking-[0.16em] text-teal-800/55 font-semibold">
        {label}
      </p>
      <p className="mt-1 font-semibold text-teal-950">{primary}</p>
      {secondary && (
        <p className="text-xs text-teal-800/65 mt-0.5">{secondary}</p>
      )}
    </div>
  );
}

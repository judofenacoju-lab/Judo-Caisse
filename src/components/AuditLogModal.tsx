...
function actionColor(action: AuditAction): string {
  if (action.includes("delete") || action === "audit_reset")
    return "bg-red-100 text-accent";
  if (action.includes("create") || action === "login")
    return "bg-green-100 text-success";
  if (action.includes("pin") || action === "export")
...
export default function AuditLogModal({ open, onClose }: AuditLogModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-card w-full max-w-3xl rounded-2xl shadow-2xl animate-fade-in max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-start justify-between gap-3 p-5 sm:p-6 border-b border-border">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              onClick={handleReset}
              disabled={resetting}
              className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-xl border border-accent/30 text-accent bg-red-50 hover:bg-red-100 disabled:opacity-50 transition-colors"
